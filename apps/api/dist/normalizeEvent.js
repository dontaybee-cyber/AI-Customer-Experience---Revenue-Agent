...
function inferChannelFromTwilio(payload) {
...
export function normalizeEvent(input) {
    ...
    if (provider === "vapi") {
        ...
        const customerExternalId = asString(p.customer?.phone) ??
    ...
    if (provider === "telegram") {
        ...
        const p = input.payload ?? {};
        const msg = p.message ?? p.edited_message ?? p.channel_post ?? p.edited_channel_post;
        if (!msg)
            throw new Error("Telegram payload missing message");
...
//# sourceMappingURL=normalizeEvent.js.map